        ...
        <div class="container-fluid" id="Category">
            <div class="heading-title p-2 my-2">
                <span class="my-3 heading "><i class="fas fa-home"></i> <a class=""
                        href="{{ route('dashboard') }}">Home</a> > Order Details</span>
            </div>
            <div class="row">
                <div class="col-md-6">
                    {{-- <ul class="list-group">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item">
                            @if ($order->status == 'Pending')
                                <a href="{{ route('admin.pending.to.confirm', $order->id) }}"
                                    class="btn btn-block btn-success shadow-none px-2 py-1" id="confirm"
                                    data-token="{{ csrf_token() }}" data-id="{{ $order->id }}">Confirm Order</a>
                                <a href="{{ route('admin.pending.to.canceled', $order->id) }}"
                                    class="btn btn-block btn-danger shadow-none px-2 py-1" id="cancel">Cancel Order</a>
                            @elseif($order->status == 'Confirm')
                                <a href="{{ route('admin.confirm.to.processing', $order->id) }}"
                                    class="btn btn-block btn-danger shadow-none px-2 py-1" id="processing">Processing</a>
                            @elseif($order->status == 'Processing')
                                <a href="{{ route('admin.processing.to.delivered', $order->id) }}"
                                    class="btn btn-block btn-danger shadow-none px-2 py-1" id="order">Delivery</a>
                            @elseif($order->status == 'Delivered')
                                <a href="{{ route('admin.delivered.to.canceled', $order->id) }}"
                                    class="btn btn-block btn-danger shadow-none px-2 py-1" id="order">Cancel
                                    Delivery</a>
                            @endif
                        </li>
