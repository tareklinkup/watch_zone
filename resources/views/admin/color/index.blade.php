        ...
        <div class="container-fluid" id="Category">
            <div class="heading-title p-2 my-2">
                <span class="my-3 heading "><i class="fas fa-home"></i> <a class=""
                        href="{{ route('dashboard') }}">Home</a> > Dial Color</span>
            </div>
            <div class="row">
                <div class="col-lg-5">
                    <div class="card my-2">
                        ...
                        <div class="card-body table-card-body">
                            <form method="post"
                                action="{{ @$colorData ? route('color.update', $colorData->id) : route('color.store') }}">
                                @csrf
                                        ...
                                        <select name="category_id" class="form-control form-control-sm shadow-none"
                                            ...
                                            @foreach ($category as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ @$colorData->category_id == $item->id ? 'selected' : '' }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-sm-9">
                                        <select name="brand_id" class="form-control form-control-sm shadow-none"
                                            ...
                                            @foreach ($brand as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ @$colorData->brand_id == $item->id ? 'selected' : '' }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group row">
                                    <label for="title" class="col-sm-3 col-form-label">Dial Color Name</label>
                                    <div class="col-sm-9">
                                        <input type="text" name="name"
                                            value="{{ @$colorData ? @$colorData->name : old('name') }}"
                                ...
                                <div class="clearfix">
                                    <div class="text-end m-auto">
                                        ...
                                        <button type="submit"
                                            class="btn btn-success shadow-none">{{ @$colorData ? 'Update change' : 'Save change' }}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                                        ...
                                        @foreach ($color as $item)
                                            <tr class="{{ $item->id }}">
                                                ...
                                                <td>
                                                    ...
                                                    <a href="{{ '/dail/color/delete/' . $item->id }}" id="delete"
