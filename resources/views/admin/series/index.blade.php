        ...
        <div class="container-fluid" id="Category">
            <div class="heading-title p-2 my-2">
                <span class="my-3 heading "><i class="fas fa-home"></i> <a class=""
                        href="{{ route('dashboard') }}">Home</a> > Series</span>
            </div>
            <div class="row">
                <div class="col-lg-5">
                    <div class="card my-2">
                        ...
                        <div class="card-body table-card-body">
                            <form method="post"
                                action="{{ @$seriesData ? route('series.update', $seriesData->id) : route('series.store') }}">
                                @csrf
                                        ...
                                        <select name="category_id" class="form-control form-control-sm shadow-none"
                                            ...
                                            @foreach ($category as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ @$seriesData->category_id == $item->id ? 'selected' : '' }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-sm-9">
                                        <select name="brand_id" class="form-control form-control-sm shadow-none"
                                            ...
                                            @foreach ($brand as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ @$seriesData->brand_id == $item->id ? 'selected' : '' }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="text" name="name"
                                            value="{{ @$seriesData ? @$seriesData->name : old('name') }}"
                                ...
                                <div class="clearfix">
                                    <div class="text-end m-auto">
                                        ...
                                        <button type="submit"
                                            class="btn btn-success shadow-none">{{ @$seriesData ? 'Update change' : 'Save change' }}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                ...
                <div class="col-lg-7">
                    <div class="card my-2">
                        <div class="card-header d-flex justify-content-between">
                            <div class="table-head"><i class="fas fa-table me-1"></i> Series List</div>
                                        ...
                                        @foreach ($series as $item)
                                            <tr class="{{ $item->id }}">
                                                ...
                                                <td>
                                                    ...
                                                    <a href="{{ '/series/delete/' . $item->id }}" id="delete"
