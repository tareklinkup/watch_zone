...
<main>
    <div class="container-fluid" id="Category">
        <div class="heading-title p-2 my-2">
            <span class="my-3 heading "><i class="fas fa-home"></i> <a class="" href="{{route('dashboard')}}">Home</a> > Faq's</span>
        </div>
        <div class="row">
            <div class="col-lg-5">
                <div class="card my-2">
                    ...
                    <div class="card-body table-card-body">
                        <form method="post" action="{{ (@$faqdata) ? route('faq.update', $faqdata->id) : route('faq.store') }}" >
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" name="qustion" value="{{ @$faqdata->qustion }}" class="form-control form-control-sm shadow-none" id="qustion">
                                    @error('qustion') <span style="color: red">{{$message}}</span> @enderror
                                </div>
                            ...
                            <div class="clearfix">
                                <div class="text-end m-auto">
                                    ...
                                    <button type="submit" class="btn btn-success shadow-none">{{ (@$faqdata)? 'Update change' : 'Save change' }}</button>
                                </div>
                            </div>
                        </form>  
                    </div>
                            ...
                            <table class="table table-bordered text-center" id="datatablesSimple" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    ...
                                    <tr class="{{ $item->id }}">
                                        ...
                                        <td>{{ $item->qustion }}</td>
                                        <td>
                                            <a href="{{ route('faq.edit', $item->id) }}" class="btn btn-edit shadow-none"><i class="fas fa-pencil-alt"></i></a>
                                            <a href="{{ route('faq.delete') }}" id="delete" data-token="{{csrf_token()}}" data-id="{{$item->id}}" class="btn btn-delete shadow-none"><i class="fa fa-trash"></i></a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
